...
/**
 * How trustworthy the time-series portion of a results set is:
 * - 'native-binary': parsed from the SWMM binary .out file — genuine engine time series.
 * - 'report-summary': only the .rpt text report was available. Summary tables and
 *   continuity errors are real, but NO time series exist (none are fabricated).
 * - 'synthetic-mock': generated by the Mock engine for interface testing only.
 */
export type ResultsFidelity = 'native-binary' | 'report-summary' | 'synthetic-mock';
...
export interface SimulationResults {
  ...
  /** Consolidated LID detail report (.lid) text — SWMM5 WASM runs whose model requests detailed LID reporting. */
  lidReportText?: string;
