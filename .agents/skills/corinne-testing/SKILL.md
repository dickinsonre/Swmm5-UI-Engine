...
The three scripts here were written to the specification above and re-verified
on 2026-09-01 inside SWMM5-UI. What was actually run, so the claims stay
separable from hers:

- `table-consistency.js` reproduces both documented rows **exactly** —
  CIRCULAR median 0.7 %, past 10 % across 5 % of the range, −34 % / +22 % at
  the y/D = 0.02 knot; GOTHIC 14.4 %, 65 %, +202 %. The constants are read
  from `swmm-engine/Stormwater-Management-Model-5.2.4/src/solver/xsect.dat` by
  `scripts/gen-xsect-tables.cjs` rather than transcribed, so they trace to the
  engine's own source. Reproducing GOTHIC requires inverting the depth-of-area
  table *exactly* — its breakpoints land on the table's y values and are
  unequally spaced. Resampling area onto an even grid first smears the answer
  to 13.1 % and quietly loses the finding.
- `out-diff.js` was verified against `tests/fixtures/verify-network.inp` run
  through the vendored 5.2.4 binary, not OWA Example1, which is not in this
  repo. Her Example1 numbers are therefore **not** reproduced here and should
  not be quoted as if they were. On the local pair (every conduit n 0.010 →
  0.013) routing continuity moved −0.018 % → −0.019 % — "nothing happened" —
  while node depths diverged up to 14.5 % of their own peak. The
  peak-magnitude and shape-and-timing rows separated as expected for a
  roughness change. A against a copy of A reports byte-identical and stops.
- `interleave-timing.js` with the same command on both sides reports
  UNRESOLVED with a straddling per-pair range, which is its null-effect
  control.

### One more trap, found writing these

The `.out` input-property block **mixes integer and float fields inside one
record**: where the first property code is `INPUT_TYPE_CODE`, the engine writes
that slot as an INT4 and the remaining slots as REAL4. The record length is
identical either way, so reading the type slot as a float leaves every later
offset correct and silently yields 0 — a weir (type 3) becomes a float32
denormal that rounds to zero and is counted as a conduit. The "conduits only"
control then quietly includes exactly the links it exists to exclude. Read that
slot as an INT4; the same applies to the node-type slot.
