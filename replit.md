...
- `client/src/lib/roundtrip-audit.ts` + `client/src/components/swmm/RoundTripAuditDialog.tsx` — In-app Round-Trip Audit (Project toolbar "Audit" button): exports current model via projectToInp, re-parses, deep-diffs normalized forms; shows omitted/altered/added fields, pass-through raw sections, fully preserved sections
...
- Calibration File Creator — integrated in calibration dialog (Create File tab), supports variable selection (node depth/head/flooding, link flow/velocity/depth, subcatchment runoff/rainfall), template generation from simulation timesteps, CSV import, .dat file export, "Fill Blanks from Results". All-parameters mode: "All parameters (all categories)" checkbox + Select All/Clear expand template to every (location × allowable variable) pair (per-row `varKey` on CalibrationEntry); one .dat file per variable on download/load (per-file download buttons shown when >1); table render capped at 300 rows; location cells are text inputs backed by 3 shared datalists (perf for large models); template interval respected for ≥60 min steps; confirm guard when projected rows >20k
